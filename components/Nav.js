  ...
  return (
    <div className="w-full fixed bottom-8 flex justify-center z-50">
        <nav className="bg-white flex gap-2 w-max z-50 px-6 py-2 rounded-full shadow-md">
            ...
            <a href="#contact" onClick={() => setActiveNav('#contact')} className={ activeNav === '#contact' ? 'active bg-black-500 p-4 rounded-full' : 'p-4 rounded-full hover:bg-gray-300' } title="Kontakt"><BiMessageSquareDetail  className={ activeNav === '#contact' ? 'text-white w-5 h-5' : 'w-5 h-5' } /></a>
        </nav>
    </div>
  )
}
